...
export class CalculatorLogicBase {
    dealerSettings;
    dealerData = null;
    standData = null;
    hitData = null;
    ...
    runDealerSim(normalize) {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            const probabilities = [];
            this.dealerOutcomeGenerator([upcard], outcomes, handProbs, probabilities, newShoe, false);
            upCardOutcomes.push(this.getDealerOutcomeCounts(outcomes, this.getTotalProbabilities(probabilities), normalize));
        }
     ...
     */
    runDealerSimGivenCards(cards, dealerUpcard, excludeCards) {
        let shoe = this.genShoe();
        const isBlackjack = this.total(cards) === 21 && cards.length === 2 && !excludeCards;
        let prob = 1;
        if (excludeCards)
            this.removeCardsFromShoe(shoe, excludeCards);
        this.removeCardsFromShoe(shoe, cards, prob);
        this.removeCardsFromShoe(shoe, [{ rank: dealerUpcard }], prob);
        const outcomes = [];
        const probabilities = [];
        this.dealerOutcomeGenerator([{ rank: dealerUpcard }], outcomes, [prob], probabilities, shoe, isBlackjack);
        const result = this.getDealerOutcomeCounts(outcomes, this.getTotalProbabilities(probabilities));
     ...
     */
    runHandSim(totalTarget, upCard, softHands) {
        const allHands = [];
        ...
        for (let playerRank = 1; playerRank <= 10; playerRank++) {
            ...
            const playerIndex = shoeAfterPlayer.findIndex((c) => c.rank === playerRank);
            const playerCard = shoeAfterPlayer.splice(playerIndex, 1)[0];
            const probPlayer = playerCount / baseShoe.length;
            const upCardCount = shoeAfterPlayer.filter((c) => c.rank === upCard).length;
            if (upCardCount === 0)
                continue;
            ...
            const recurse = (hand, shoe, handProbs, minRank) => {
                let total = this.total(hand);
                const isSoft = this.isSoft(hand);
                if (!softHands && isSoft && totalTarget > 11)
                    total -= 10;
                    ...
                    if ((softHands && isSoft) || !softHands) {
                        ...
                        if (!seenCombos.has(key)) {
                            ...
                            for (let nextRank = 1; nextRank <= 10; nextRank++) {
                                const count = remainingShoe.filter((c) => c.rank === nextRank).length;
                                if (count === 0)
                                    continue;
     ...
     */
    getDealerData() {
        return this.getDataSet(this.dealerData.outcomes);
    }
    /**
     ...
     */
    getCumulativeProbs(decision) {
        let data = decision === "stand"
            ? this.standData
            : decision === "hit"
                ? this.hitData
                : decision === "double"
        ...
        for (let upCard = 1; upCard <= 10; upCard++) {
            ...
            for (let totalTarget = 4; totalTarget <= 21; totalTarget++) {
                const candidateHands = this.runHandSim(totalTarget, upCard, false).allHands;
                let EVs = [];
                let probs = [];
                for (let hand of candidateHands) {
                    if (decision === "double" && hand.hand.length > 2)
                        continue;
                    const handTotal = this.total(hand.hand);
                    if (handTotal === totalTarget) {
                        if (decision === "stand") {
                            EVs.push(this.calcStandEV(hand.hand, this.getData(hand.hand, upCard, data)));
                        }
                        else if (decision === "hit") {
                            EVs.push(this.calcHitEV(this.getData(hand.hand, upCard, data)));
                        }
                        else if (decision === "double") {
                            EVs.push(this.calcDoubleEV(this.getData(hand.hand, upCard, data)));
                        }
                        else {
        ...
        for (let upCard = 1; upCard <= 10; upCard++) {
            ...
            for (let totalTarget = 12; totalTarget <= 21; totalTarget++) {
                const candidateHands = this.runHandSim(totalTarget, upCard, true).allHands;
                let EVs = [];
                let probs = [];
                for (let hand of candidateHands) {
                    if (decision === "double" && hand.hand.length > 2)
    ...
    getSplitProbs() {
        const splits = [];
        let dataSet = this.getDataSet(this.splitData.EV);
        for (let upCard = 1; upCard <= 10; upCard++) {
            const upcardResults = [];
            for (let totalTarget = 1; totalTarget <= 10; totalTarget++) {
                let EV = -99;
                const handIndex = this.getHandIndex([{ rank: totalTarget }, { rank: totalTarget }], dataSet[upCard - 1]);
                if (handIndex !== -1) {
     ...
     */
    isBlackjack(cards, excludeCards) {
     ...
     */
    calcStand(cards, upCard, excludeCards) {
        ...
        const dealerProbs = this.runDealerSimGivenCards(cards, upCard, excludeCards);
     ...
     */
    calcStandEV(hand, stand, excludeCards) {
     ...
     */
    calcStandVariance(hand, stand, excludeCards) {
        if (this.isBlackjack(hand, excludeCards)) {
     ...
     */
    calcHit(cards, upCard, excludeCards) {
        ...
        for (let nextRank = 1; nextRank <= 10; nextRank++) {
            ...
            if (shoe.filter((c) => c.rank === nextRank).length === 0)
                continue;
     ...
     */
    calcDouble(cards, upCard, excludeCards) {
        let winProb = 0;
        let tieProb = 0;
        let loseProb = 0;
        let DBJ = 0;
     ...
     */
    calcSplit(cards, upCard, excludeCards) {
        let handProbs = {
        ...
        const total = handProbs.double.winProb +
            handProbs.double.tieProb +
            handProbs.double.loseProb +
    ...
    getNextCardProb(shoe, upCard) {
        ...
        for (let nextRank = 1; nextRank <= 10; nextRank++) {
            if ((upCard != 10 && upCard != 1) || this.dealerSettings.ENHC) {
                ...
                if (nextRank === 1)
                    nextCardProbs.push(aceCount / (cardsRemaning - 1));
                else {
                    nextCardProbs.push((count * (1 - 1 / (cardsRemaning - aceCount))) /
                        (cardsRemaning - 1));
                }
            }
            else if (upCard === 1) {
                ...
                const cardsRemaning = shoe.length;
                if (nextRank === 10)
                    nextCardProbs.push(tenCount / (cardsRemaning - 1));
                else {
                    nextCardProbs.push((count * (1 - 1 / (cardsRemaning - tenCount))) /
                        (cardsRemaning - 1));
                }
            }
        }
     ...
     */
    removeCardsFromShoe(shoe, cards, prob) {
        for (let card of cards) {
            const count = shoe.filter((c) => c.rank === card.rank).length;
            if (count === 0)
                throw new Error(`No ${card.rank} card found in shoe!`);
            if (prob)
                prob *= count / shoe.length;
     ...
     */
    dealerOutcomeGenerator(dealerHand, outcomes, handProbs, probabilities, shoe, playerBJ) {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            this.dealerOutcomeGenerator(newCards, outcomes, newProbabilities, probabilities, newShoe, playerBJ);
        }
    }
    /**
     ...
     */
    getTotalProbabilities(probabilities) {
     ...
     */
    getDealerOutcomeCounts(outcomes, probabilities, normalize) {
