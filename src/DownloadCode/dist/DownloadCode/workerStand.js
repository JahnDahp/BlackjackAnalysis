...
console.log(`Starting stand: ${decks}D ${S17 ? "S17" : "H17"} ${ENHC ? "ENHC" : "US"}`);
const hard = [];
        ...
        for (const hand of candidateHands) {
            if (instance.total(hand.hand) === totalTarget) {
                upcardResults.push([
                    hand.hand,
                    hand.totalProb,
                    instance.calcStand(hand.hand, upCard),
                ]);
            }
        }
    }
...
for (let upCard = 1; upCard <= 10; upCard++) {
    ...
    for (let totalTarget = 12; totalTarget <= 21; totalTarget++) {
        const candidateHands = instance.runHandSim(totalTarget, upCard, true).allHands;
        for (const hand of candidateHands) {
            if (instance.total(hand.hand) === totalTarget) {
                upcardResults.push([
                    hand.hand,
