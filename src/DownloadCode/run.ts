...
const configs: { decks: number; S17: boolean; ENHC: boolean }[] = [];
for (const decks of [1, 2, 4, 6, 8])
  for (const S17 of [false, true])
    for (const ENHC of [false, true]) configs.push({ decks, S17, ENHC });
...
const promises = configs.map(({ decks, S17, ENHC }) => {
  return new Promise<void>((resolve, reject) => {
    ...
    worker.on("error", (err) => {
      console.error(
        `Worker error (${decks}D ${S17 ? "S17" : "H17"} ${ENHC ? "ENHC" : "US"}):`,
        err,
      );
      reject(err);
    });
...
Promise.all(promises).then(() => {
  const byDecks: Record<number, any> = {};
