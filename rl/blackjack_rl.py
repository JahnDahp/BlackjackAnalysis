# Run with: python blackjack_rl.py [--decks N] [--s17|--h17] [--enhc] [--das|--ndas] [--surrender|--no-surrender] [--episodes N]
...
class QTable:
  ...
  def rebuild_strategy(self) -> None:
    for total in range(4, 22):
      self.update_strategy_for("hard", total)
    for total in range(13, 22):
      self.update_strategy_for("soft", total)
    self._dirty = False

  def update_strategy_for(self, table: str, key: int) -> None:
    acts = [self.best_action(table, key, upcard) for upcard in range(1, 11)]
    modal = max(set(acts), key=acts.count)
    fallback = HIT if modal in (DOUBLE, SURRENDER) else modal
    if table == "hard":
      self._hard_dbl[key] = HIT if modal == SURRENDER else modal
      self._hard_nodbl[key] = fallback
    else:
      self._soft_dbl[key] = HIT if modal == SURRENDER else modal
      self._soft_nodbl[key] = fallback
...
def train(
  ...
  weights = upcard_weights(rules.decks)
    ...
    for hand in rl_df.index:
      ...
      for col in rl_df.columns:
        if col not in verified_df.columns:
          continue
        rl_val = str(rl_df.loc[hand, col]).strip()
        ver_val = str(verified_df.loc[v_hand, col]).strip()
        if rl_val != ver_val:
